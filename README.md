...
본 프로젝트는 분석 과정에서 **인과 분석 → 예측 분석으로 질문을 재정의**했습니다. 상관분석·부분상관·머신러닝 세 단계가 일관되게 "시설물은 사고의 원인이 아니라 위험 지역에 사후 설치되는 지표이며, 사고 위험을 결정하는 핵심은 교통량"이라는 결론으로 수렴했기 때문입니다.
...
> **신호등 제외 사유**: 신호등 데이터가 일부 지역에만 존재하여 전국 분석에 편향을 유발하므로 제외했습니다. (모델 중요도 최하위권으로 성능 영향 미미)
...
1. **횡단면 데이터** — 시설물 설치 시점 데이터 부재로 인과 방향 직접 검증 불가
2. **역인과 가능성** — 무인단속카메라 등은 사고 다발 구역에 사후 설치되는 경향
3. **데이터 가용성** — 신호등은 일부 지역만 존재하여 전국 편향 유발 → 분석에서 제외
4. **클래스 불균형** — 안전 등급 72.2%, SMOTE로 완화했으나 '주의' 등급은 개선 여지 존재
5. **공간 자기상관** — 보호구역 간 반경 겹침으로 완전한 독립성 미보장
